...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//设置bean表达式解析器，spring3.0增加了 EL表达式的支持
		...
		// 从而在Aware接口实现类中的注入applicationContext,在BEAN初始化之前，
		// 调用ApplicationContextAwareProcessor的postProcessBeforeInitialization
	...
	protected void initMessageSource() {
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			...
			//如果用户没有定义配置文件，那么使用临时的DelegatingMessageSource以便于作为调用getMessage()
			// 方法的返回
	...
	protected void initApplicationEventMulticaster() {
		...
		// 没有
			...
			// 自己创建个默认的SimpleApplicationEventMulticaster
			this.applicationEventMulticaster = new SimpleApplicationEventMulticaster(beanFactory);
			// 注册一下，不让没法用，上面的不用注册，是可以从beanFactory里面获取，说明注册过了
	...
	protected void registerListeners() {
		...
		// 再注册实现了ApplicationListener的bean
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// Instantiate all remaining (non-lazy-init) singletons.
		//对没有配置了lazy-init属性的单态模式Bean进行预实例化处理

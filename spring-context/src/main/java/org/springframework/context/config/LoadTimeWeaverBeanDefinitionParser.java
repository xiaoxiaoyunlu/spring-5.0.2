...
class LoadTimeWeaverBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
	...
	protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder) {
		...
		// 判断是否支持 aspectj-weaving
		if (isAspectJWeavingEnabled(element.getAttribute(ASPECTJ_WEAVING_ATTRIBUTE), parserContext)) {
			// 如果没有解析到对应的 internalAspectJWeavingEnabler
			if (!parserContext.getRegistry().containsBeanDefinition(ASPECTJ_WEAVING_ENABLER_BEAN_NAME)) {
				// 创建对应的 beandefinition 并注册
				RootBeanDefinition def = new RootBeanDefinition(ASPECTJ_WEAVING_ENABLER_CLASS_NAME);
				parserContext.registerBeanComponent(
						new BeanComponentDefinition(def, ASPECTJ_WEAVING_ENABLER_BEAN_NAME));
			}
			// 如果已经存在，则判断是否否是 AnnotationBeanConfigurerAspect
			if (isBeanConfigurerAspectEnabled(parserContext.getReaderContext().getBeanClassLoader())) {
				// 是的哈 采用 SpringConfiguredBeanDefinitionParser
				new SpringConfiguredBeanDefinitionParser().parse(element, parserContext);
			}
		}
	}

	// 判断是否支持 aspectj-weaving 可以配置如下： on   off   autodetect

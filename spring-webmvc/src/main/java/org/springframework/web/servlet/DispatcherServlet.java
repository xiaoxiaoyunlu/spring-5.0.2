...
public class DispatcherServlet extends FrameworkServlet {
	 ...
	 */
	// dispatcherServlet 的 核心初始化入口
	protected void initStrategies(ApplicationContext context) {
		// MultipartResolver 文件上传配置处理
		initMultipartResolver(context);
		// 国际化配置
		initLocaleResolver(context);
		// 主题资源配置，不常用
		initThemeResolver(context);
		// URL handlerMapper 映射
	 ...
	 */
	// 初始化 URL handler  映射关系
	private void initHandlerMappings(ApplicationContext context) {
		this.handlerMappings = null;

		// 默认加载所有的 handlerMapping
		if (this.detectAllHandlerMappings) {
			...
			if (!matchingBeans.isEmpty()) {
				...
				// handlerMapping 优先级 ，所以要比较sort
				AnnotationAwareOrderComparator.sort(this.handlerMappings);
			}
		}
		// 参数 detectAllHandlerMappings = false 说明，只希望加载指定的 handlerMapping
		else {
			try {
				// 指定的 单个
	...
	private void initHandlerAdapters(ApplicationContext context) {
		...
		// detectAllHandlerAdapters  = false 加载指定的 HandlerAdapter
		else {
			try {
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				//4. 缓存处理 last-modified 请求头 如果支持的话
				...
				if (isGet || "HEAD".equals(method)) {
					...
					// 对于GET HEADER 请求，如果没有修改，则直接返回，无需处理
					if (new ServletWebRequest(request, response).checkNotModified(lastModified) && isGet) {
						return;
					}
				}

				//5 拦截器处理，返回false, 说明 拦截器处理结束，直接返回
				if (!mappedHandler.applyPreHandle(processedRequest, response)) {
					return;
				}

				// Actually invoke the handler.
				// 6.实际的处理器处理请求,返回结果视图对象
	...
	protected HttpServletRequest checkMultipart(HttpServletRequest request) throws MultipartException {
		if (this.multipartResolver != null && this.multipartResolver.isMultipart(request)) {
			if (WebUtils.getNativeRequest(request, MultipartHttpServletRequest.class) != null) {
				...
				try {
					// 把request 处理成  MultipartHttpServletRequest
	...
	protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		if (this.handlerMappings != null) {
			for (HandlerMapping hm : this.handlerMappings) {
				...
				// 返回一个 带有拦截器链的 handler 组合为HandlerExecutionChain
	 ...
	 */
	// 根据handler 找 对应的  HandlerAdapter
	protected HandlerAdapter getHandlerAdapter(Object handler) throws ServletException {
		if (this.handlerAdapters != null) {
			for (HandlerAdapter ha : this.handlerAdapters) {
				...
				// 主要逻辑 查看SimpleControllerHandlerAdapter实现

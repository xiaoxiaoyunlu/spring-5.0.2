...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected WebApplicationContext initWebApplicationContext() {
		// 从ServletContext中获取父容器 WebApplicationContext
		...
		if (this.webApplicationContext != null) {
			...
			// 是不是 ConfigurableWebApplicationContext 类型的 context
			if (wac instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					// 说明没有初始化context, 那么就 refreshed()
					configureAndRefreshWebApplicationContext(cwac);
				}
			}
		}
		// 说明 没有符合条件的 application context
		if (wac == null) {
			// No context instance was injected at construction time -> see if one
			// has been registered in the servlet context. If one exists, it is assumed
			// that the parent context (if any) has already been set and that the
			// user has performed any initialization such as setting the context id
			// 使用 contextAttribute属性加载 WebApplicationContezt
	 ...
	 * @see org.springframework.web.context.support.XmlWebApplicationContext
	 */
	// 根据root context  创建默认的context
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		// 获取servlet 的初始化参数 contextClass, 如果没有配置则默认XmlWebApplicationContext
		...
		// 反射方式 实例化 contextClass
		ConfigurableWebApplicationContext wac =
				(ConfigurableWebApplicationContext) BeanUtils.instantiateClass(contextClass);
		// 配置 environment
		wac.setEnvironment(getEnvironment());
		wac.setParent(parent);
		// 获取 servlet 属性参数  contextConfigLocation
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		// 手动刷新  覆写了 AbstractApplicationContext.refresh()方法
		wac.refresh();
	}
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// 1.为了保证，当前线程的LocaleContext和RequestAttributes属性，在请求结束后，还能恢复，
		// 提取取当前线程的属性 previousLocaleContext
		LocaleContext previousLocaleContext = LocaleContextHolder.getLocaleContext();

		// 2. 根据当前request 创建  localeContext
		LocaleContext localeContext = buildLocaleContext(request);
		// 3 提取当前线程的属性  previousAttributes
		RequestAttributes previousAttributes = RequestContextHolder.getRequestAttributes();
		// 4. 根据当前request, 创建 requestAttributes
		...
		try {
			// 6.委托子类实现，查看DispatcherServlet.service()

...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// 获取 beanFactory 里面的 TransactionManager
		final PlatformTransactionManager tm = determineTransactionManager(txAttr);
		// 构造方法唯一识别标识：比如 service.UserService.impl.save
		final String joinpointIdentification = methodIdentification(method, targetClass, txAttr);

		// 不是CallbackPreferringPlatformTransactionManager
		// 所以不是编程式事务，这里是 声明式事物的处理逻辑
		// 声明式事务 包括xml方式和注解方式
		if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			try {
				...
				// 真正调用的是 TransactionInteceptor chain 里面的 方法
		...
		// 编程式事务处理
			...
			try {
				Object result = ((CallbackPreferringPlatformTransactionManager) tm).execute(txAttr, status -> {
					...
					try {
						// 事务增强调用 调用的是 AbstractTransactionAspect
						return invocation.proceedWithInvocation();
					}
					catch (Throwable ex) {
						if (txAttr.rollbackOn(ex)) {
							// A RuntimeException: will lead to a rollback.
							// 只对RuntimeException 异常进行回滚
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		// 如果没有指定名称，则使用方法唯一标识符，并使用DelegatingTransactionAttribute封装txAttr
	 ...
	 */
	// 根据给定的信息，准备事务信息 TransactionInfo
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		...
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			// 判断 是否回滚 默认的依据是  抛出的异常，是狗屎RuntimeException 或者 Error 类型

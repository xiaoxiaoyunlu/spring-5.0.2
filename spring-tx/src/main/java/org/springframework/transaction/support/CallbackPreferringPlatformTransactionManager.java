 ...
 * // 编程式事务的回调处理（commit()  or  rollback()）
 * callbacks over programmatic {@code getTransaction}, {@code commit}
 * and {@code rollback} calls.
 * // 可用是否是实现CallbackPreferringPlatformTransactionManager来判断是不是编程式事务
 * Calling code may check whether a given

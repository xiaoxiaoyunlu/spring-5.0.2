...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(final String beanName, final RootBeanDefinition mbd,
		...
		//getBean方法调用的时候，有没有指定方法参数，有的话，直接用
		if (explicitArgs != null) {
			argsToUse = explicitArgs;
		}
		// 没有，就得解析出参数，来匹配构造函数了
		else {
			// 尝试从配置文件中解析参数，配置文件中的都会解析成RootBeanDefinition,也就是mbd里面
			...
			if (argsToResolve != null) {
				// 解析参数类型 ，缓存中的可能是原值，也可能是最终值，
				// 比如A（int,int）则会把（'1','1'）转换为(1,1,)
		...
		if (constructorToUse == null) {
			...
			for (Constructor<?> candidate : candidates) {
				...
				// 如果已经找到选用的构造函数，或者需要的参数个数小于当前构造函数个数，则终止
				// 因为已经按照参数个数降序排列
				...
				// 探测是否有不确定的构造函数存在，例如不同构造函数的参数为父子类型
				int typeDiffWeight = (mbd.isLenientConstructorResolution() ?
						argsHolder.getTypeDifferenceWeight(paramTypes) : argsHolder.getAssignabilityWeight(paramTypes));
				// Choose this constructor if it represents the closest match.
				// 判断，这个构造函数是匹配最接近的构造函数
